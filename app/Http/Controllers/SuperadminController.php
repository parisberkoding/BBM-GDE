...
class SuperadminController extends Controller
{
    ...
    public function exportCSV(Request $request)
    {
        try {
            ...
            $query = Permohonan::with(['requester', 'consumerialTool', 'transactionProof', 'approvedRequest']);

            // Apply filters (same as pivot table)
            if ($request->has('status_filter')) {
            ...
            $columnMapping = [
                ...
                'driver_name' => 'Nama Pemohon',
                'vehicle' => 'Kendaraan',
                'plat_nomer' => 'Plat Nomor',
                'merk' => 'Merk',
                'jenis' => 'Jenis',
                'gasoline_type' => 'Jenis BBM',
                'bill_amounts' => 'Nominal',
            ...
            foreach ($permohonans as $permohonan) {
                ...
                foreach ($selectedColumns as $col) {
                    switch ($col) {
                        case 'request_number':
                            $row[] = $permohonan->request_number;
                            break;
                        case 'driver_name':
                            $row[] = $permohonan->requester->fullname ?? 'Unknown';
                            break;
                        case 'vehicle':
                            $vehicle = $permohonan->consumerialTool;
                            $row[] = $vehicle ? "{$vehicle->plat_nomer} - {$vehicle->merk}" : '';
                            break;
                        case 'plat_nomer':
                            $row[] = $permohonan->consumerialTool->plat_nomer ?? '';
                            break;
                        case 'merk':
                            $row[] = $permohonan->consumerialTool->merk ?? '';
                            break;
                        case 'jenis':
                            $row[] = $permohonan->consumerialTool->jenis ?? '';
                            break;
                        case 'gasoline_type':
                            $row[] = $permohonan->gasoline_type;
                            break;
                        case 'bill_amounts':
                            $row[] = $permohonan->bill_amounts;
                            break;
                        case 'request_date':
                            $row[] = $permohonan->request_date->format('Y-m-d H:i:s');
                            break;
                        case 'status':
                            $row[] = ucfirst($permohonan->status);
                            break;
                        case 'voucher_code':
                            $row[] = $permohonan->approvedRequest->voucher_code ?? '';
                            break;
                        case 'proof_id':
                            $row[] = $permohonan->transactionProof->transaction_id ?? '';
                            break;
                        case 'tanggal_pengisian':
                            $proof = $permohonan->transactionProof;
                            $row[] = $proof ? $proof->purchase_datetime->format('Y-m-d H:i:s') : '';
                            break;
                        case 'volume':
                            $row[] = $permohonan->transactionProof->fuel_volume ?? 0;
                            break;
                        case 'km_terakhir':
                            $row[] = $permohonan->transactionProof->km_terakhir ?? 0;
                            break;
                        default:
                            $row[] = '';
                    }
            ...
            // Generate CSV file
            $filename = 'export_permohonan_' . date('Y-m-d_His') . '.csv';
    ...
    {
        // TODO: Implementasi PDF export menggunakan DomPDF atau TCPDF
        // Untuk sementara return not implemented

        return response()->json([
            'success' => false,
            'message' => 'Fitur export PDF sedang dalam pengembangan. Silakan gunakan export CSV terlebih dahulu.'
        ], 501);
    }

    // ==================== LATEST ODO PER VEHICLE ====================

    public function getLatestOdoPerVehicle()
    {
        try {
            // Get latest odometer reading per vehicle (only Kendaraan, not Alat)
            $latestOdos = Transaction_Proofs::with(['permohonan.consumerialTool', 'permohonan.requester'])
                ->whereHas('permohonan.consumerialTool', function($q) {
                    $q->where('jenis', 'Kendaraan');
                })
                ->where('km_terakhir', '>', 0)
                ->orderBy('purchase_datetime', 'desc')
                ->get()
                ->groupBy('permohonan.consumerial_tools_id')
                ->map(function ($proofs) {
                    // Get the latest proof for each vehicle
                    $latestProof = $proofs->first();
                    $vehicle = $latestProof->permohonan->consumerialTool;
                    ...
                    return [
                        ...
                        'merk' => $vehicle->merk,
                        'jenis' => $vehicle->jenis,
                        'kmTerakhir' => $latestProof->km_terakhir,
                        'tanggalUpdate' => $latestProof->purchase_datetime->toISOString(),
                        'requestNumber' => $latestProof->permohonan->request_number,
                        'driverName' => $latestProof->permohonan->requester->fullname ?? 'Unknown',
                    ];
                })
                ->sortBy('platNumber')
                ->values();
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal memuat data ODO: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function getRequestAnalysis(Request $request)
    {
        try {
            $query = Permohonan::with(['requester']);
            ...
            // Apply nominal filters
            if ($request->has('min_nominal')) {
                $query->where('bill_amounts', '>=', $request->min_nominal);
            }

            if ($request->has('max_nominal')) {
                $query->where('bill_amounts', '<=', $request->max_nominal);
            }

            $requests = $query->get();

            // Aggregate by driver
            $driverData = $requests->groupBy('requester_id')
                ->map(function ($group) {
                    $driver = $group->first()->requester;

                    return [
                        'driverName' => $driver->fullname ?? 'Unknown',
                        'count' => $group->count(),
                        'totalNominal' => $group->sum('bill_amounts'),
                        'avgNominal' => $group->avg('bill_amounts'),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal memuat analisis request: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function getMonthlyReportSummary(Request $request)
    {
        try {
            ...
            $summary = [
                ...
                'totalRequests' => Permohonan::whereBetween('request_date', [$startDate, $endDate])->count(),
                'totalExpense' => Permohonan::whereBetween('request_date', [$startDate, $endDate])
                    ->whereIn('status', ['completed', 'approved'])
                    ->sum('bill_amounts'),
                'totalVolume' => Transaction_Proofs::whereBetween('purchase_datetime', [$startDate, $endDate])
                    ->sum('fuel_volume'),
                'completedRequests' => Permohonan::whereBetween('request_date', [$startDate, $endDate])
                    ->where('status', 'completed')
                    ->count(),
                'pendingRequests' => Permohonan::whereBetween('request_date', [$startDate, $endDate])
                    ->where('status', 'pending')
                    ->count(),
                'rejectedRequests' => Permohonan::whereBetween('request_date', [$startDate, $endDate])
                    ->where('status', 'rejected')
                    ->count(),

                // Top 5 vehicles by expense
                'topVehicles' => Permohonan::with('consumerialTool')
                    ...
                    ->map(function ($item) {
                        $vehicle = $item->consumerialTool;
                        return [
                            'vehicle' => $vehicle ? "{$vehicle->plat_nomer} - {$vehicle->merk}" : 'Unknown',
                            'expense' => $item->total
                        ];
                    }),

                // Top 5 drivers by request count
                'topDrivers' => Permohonan::with('requester')
                    ->whereBetween('request_date', [$startDate, $endDate])
                    ->selectRaw('requester_id, COUNT(*) as count, SUM(bill_amounts) as total')
                    ->groupBy('requester_id')
                    ->orderBy('count', 'desc')
                    ->limit(5)
                    ->get()
                    ->map(function ($item) {
                        return [
                            'driver' => $item->requester->fullname ?? 'Unknown',
